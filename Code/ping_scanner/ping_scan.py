...
def recieved_ping_from_addresses(ID: int, timeout: float) -> List[Tuple[str, float, int]]:
    ...
    # or returns minus one if it ran out of time
        ...
        # decrease the amount of time remaining to wait for by the amount of time spent waiting
        ...
        if p_id == ID:
            # if the ping was sent from this machine then add it to the list of responses
            ip, port = addr
...
with closing(socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_ICMP)) as ping_sock:
    ...
    # get the list of addresses that replied to the echo request from the listener function
    print("\n".join(map(lambda x: f"host: [{x[0]}]\tresponded to an ICMP ECHO REQUEST in {str(round_significant_figures(x[1], 2)) + 's':<10s}ttl: [{x[2]}]", hosts_up)))
