...
while len(packets) < 1:
    ...
    # the above line deconstructs the ip_header variable into its component parts
    hl_v = f"{ip_hp_ip_v:08b}"
    ip_v = int(hl_v[:4], 2)
    ip_hl = int(hl_v[4:], 2)
    # splits hl_v in ip_v and ip_hl which store the IP version number and header length respectively
    dscp_ecn = f"{ip_dscp_ip_ecn:08b}"
    ip_dscp = int(dscp_ecn[:6], 2)
    ip_ecn = int(dscp_ecn[6:], 2)
    # splits dscp_ecn into ip_dscp and ip_ecn which are two of the compenents in an IP header
    flgs_off = f"{ip_flgs_ip_off:016b}"
    ip_flgs = int(flgs_off[:3],2)
    ip_off = int(flgs_off[3:], 2)
    # splits flgs_off into ip_flgs and ip_off which represent the ip header flags and the data offset
